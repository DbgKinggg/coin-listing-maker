...
const DEFAULT_COINS: Coin[] = [
  { id: uid(), ticker: "HUMA", fullName: "Huma Finance",    leverage: "50×", colorIdx: 0 },
  { id: uid(), ticker: "EDGE", fullName: "EdgeX Token",     leverage: "3×",  colorIdx: 1 },
  { id: uid(), ticker: "ROBO", fullName: "Robonomics",      leverage: "10×", colorIdx: 2 },
  { id: uid(), ticker: "BOTZ", fullName: "Botz Protocol",   leverage: "10×", colorIdx: 3 },
  { id: uid(), ticker: "MAGS", fullName: "Magnus Network",  leverage: "10×", colorIdx: 4 },
  { id: uid(), ticker: "URA",  fullName: "Uranium DAO",     leverage: "10×", colorIdx: 5 },
];
...
function OrbCircle({ size, colorIdx, ticker }: { size: number; colorIdx: number; ticker: string }) {
  const g = GRADIENTS[colorIdx % GRADIENTS.length];
  const fs = Math.round(size * 0.38);
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", flexShrink: 0,
      background: `radial-gradient(circle at 35% 30%, ${g.from}, ${g.to} 70%)`,
      boxShadow: `0 ${size * 0.17}px ${size * 0.45}px rgba(0,0,0,0.45), inset 0 ${size * 0.052}px ${size * 0.17}px rgba(255,255,255,0.18), inset 0 ${-size * 0.086}px ${size * 0.31}px rgba(0,0,0,0.35)`,
...
function M1({ coins }: { coins: Coin[] }) {
  ...
  while (shown.length < 6) shown.push({ id: "", ticker: "—", fullName: "", leverage: "", colorIdx: shown.length % GRADIENTS.length });
  return (
      ...
      <div style={{ position: "absolute", left: 60, right: 60, top: 380, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, zIndex: 4 }}>
        {shown.map((c, i) => (
          <div key={c.id || i} style={{ padding: "22px 26px", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, display: "flex", alignItems: "center", gap: 20, position: "relative", overflow: "hidden" }}>
            ...
            <OrbCircle size={96} colorIdx={c.colorIdx} ticker={c.ticker} />
        ...
        <div style={{ background: "rgba(12,10,10,0.6)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, overflow: "hidden", backdropFilter: "blur(6px)", display: "flex", flexDirection: "column" }}>
          ...
          {shown.map((c, i) => (
            <div key={c.id || i} style={{ display: "grid", gridTemplateColumns: "72px 1.1fr 1.4fr 110px", gap: 18, alignItems: "center", padding: "10px 26px", borderBottom: i < shown.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
              <OrbCircle size={52} colorIdx={c.colorIdx} ticker={c.ticker} />
    ...
    <div style={{ width: CW, height: CH, position: "relative", overflow: "hidden",
      ...
      {shown.map((c, i) => (
        <div key={c.id || i} style={{ position: "absolute", display: "flex", alignItems: "center", gap: 14, padding: "12px 22px 12px 12px", background: "rgba(18,14,14,0.92)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 999, fontFamily: "Inter, sans-serif", zIndex: 4, boxShadow: "0 18px 40px rgba(0,0,0,0.5)", ...positions[i] }}>
          <OrbCircle size={56} colorIdx={c.colorIdx} ticker={c.ticker} />
...
export default function MultiPage() {
  ...
  function addCoin() {
    ...
    setCoins(prev => [...prev, { id: uid(), ticker: "NEW", fullName: "New Token", leverage: "10×", colorIdx: prev.length % GRADIENTS.length }]);
  }
  ...
  function updateCoin(id: string, field: keyof Omit<Coin, "id">, value: string | number) {
                ...
                return (
                  <div key={c.id} className="bg-[#14141a] border border-white/[0.07] rounded-xl p-3 space-y-2">
                    <div className="flex items-center gap-2">
                      {/* Color swatch */}
                      <div className="h-8 w-8 rounded-full shrink-0 flex items-center justify-center text-white text-xs font-black" style={{ background: `radial-gradient(circle at 35% 30%, ${g.from}, ${g.to} 70%)` }}>
                    ...
                    <div className="flex gap-1.5">
                      {GRADIENTS.map((gr, gi) => (
                        <button key={gi} onClick={() => updateCoin(c.id, "colorIdx", gi)}
                          className={`h-5 flex-1 rounded transition-all border-2 ${c.colorIdx === gi ? "border-white scale-105" : "border-transparent hover:scale-105"}`}
